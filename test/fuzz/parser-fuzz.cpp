...
void fuzzChunkedContiguous(const std::uint8_t *data, std::size_t size) {
    ...
    std::string_view remaining(
        size ? reinterpret_cast<const char *>(data) : &empty,
        size);
...
struct WebSocketFuzzImplementation {
    ...
    static bool refusePayloadLength(
        std::uint64_t length,
        uWS::WebSocketState<true> *,
        void *) {
...
using FuzzedWebSocketProtocol =
    uWS::WebSocketProtocol<true, WebSocketFuzzImplementation>;

void consumeWebSocketSegment(
    const std::uint8_t *data,
    std::size_t size,
    uWS::WebSocketState<true> &state,
    WebSocketFuzzUser &user) {
    constexpr std::size_t PRE_PADDING =
        FuzzedWebSocketProtocol::CONSUME_PRE_PADDING;
    constexpr std::size_t POST_PADDING = 32;
    std::vector<char> buffer(PRE_PADDING + size + POST_PADDING);

    if (size) {
        std::memcpy(buffer.data() + PRE_PADDING, data, size);
    }
    FuzzedWebSocketProtocol::consume(
        buffer.data() + PRE_PADDING,
...
void fuzzWebSocketByteSplit(const std::uint8_t *data, std::size_t size) {
    ...
    for (std::size_t index = 0;
         index < std::min(size, MAX_SPLIT_INPUT) && !user.closed;
         index++) {
        consumeWebSocketSegment(data + index, 1, state, user);
    }
}
